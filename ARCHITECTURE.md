...
- **`CabinScene`** / **`CaptainRoom`** (side-scroller ship rooms) reuse PlayerCharacter + the `InteractionDetector` / `Interactable` pair; `Interactable.gray_box = false` = invisible area over real art (station = logic-parent, sprite-child).Ship rooms connect via `SceneRouter.change_screen(scene, spawn)`(edgeArea2Dzones + named spawn markers) with per-room `Camera2D`; `GameManager._process` flows time in SHIP/GARDEN. The old `top-down KitchenScene`/`TopDownActor`, `ShipScreen`, and `MorningScreen` are now orphaned.
