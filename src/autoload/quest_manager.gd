...
func get_current_quest_text() -> String:
	return tr(_QUEST_TEXT.get(GameState.quest_phase, "Explore the seas."))
...
func grant_recipe(recipe_id: StringName, substitute_coins: int = 25) -> void:
	...
	var dish_name := tr(rec.display_name) if rec else String(recipe_id).capitalize()
	if GameState.mark_recipe_known(recipe_id):
		...
		UIManager.show_notice(tr("Already Known"), tr("You already know how to make %s!\nHere are %d coins instead.") % [dish_name, substitute_coins])